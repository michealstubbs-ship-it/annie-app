...
describe('listMeetingsWithContacts', () => {
  it('joins the linked contact and candidate, team-scoped by RLS, newest first, no client-side user_id filter', async () => {
  ...
  it('throws instead of silently returning [] when Supabase reports an error', async () => {
    ...
    fromMock.mockReturnValue(builder)
    await expect(listMeetingsWithContacts('user_1')).rejects.toEqual({ message: 'db down' })
  })
})
