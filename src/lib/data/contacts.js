...
// 2026-09-03, Michael: "in case there are any ownerships" — same reasoning
// as candidates.js's findCandidateDuplicateByEmail, for the same reason:
// two team members can independently add the same person as a contact with
// no warning today. Deliberately email-only and team-wide (not scoped to
// one company like findContactIdByCompanyAndName above) — a real duplicate
// contact is just as likely to show up attached to no company, or a
// slightly different one, as to the exact same company_id.
export async function findContactDuplicateByEmail(email) {
